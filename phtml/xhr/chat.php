...
if ($f == 'chat') {
    ...
    if ($s == 'chat_side') {
        if (Wo_CheckMainSession($hash_id) === true) {
           $online_users  = '';
            $offline_users = '';
            $RecentChatUsers = Wo_GetMessagesUsers($wo['user']['user_id'], '', 50, false, 0, 'recent');
            $OnlineUsers   = array();
            $OfflineUsers  = array();
            if (!empty($RecentChatUsers)) {
                foreach ($RecentChatUsers as $chat_user) {
                    if (empty($chat_user['user_id'])) {
                        continue;
                    }
                    if (Wo_IsOnline($chat_user['user_id']) === true) {
                        $OnlineUsers[] = $chat_user;
                    } else {
                        $OfflineUsers[] = $chat_user;
                    }
                }
            }
            $count_chat    = count($OnlineUsers);
    ...
    if ($s == 'update_online_recipients') {
       $html        = '';
        ...
        if (!empty($RecentChatUsers)) {
            foreach ($RecentChatUsers as $chat_user) {
                if (empty($chat_user['user_id']) || Wo_IsOnline($chat_user['user_id']) !== true) {
                    continue;
                }
                $OnlineUsers[] = $chat_user;
            }
    ...
    if ($s == 'load_chat_messages') {
        if (!empty($_GET['recipient_id']) && is_numeric($_GET['recipient_id']) && $_GET['recipient_id'] > 0 && Wo_CheckMainSession($hash_id) === true) {
            $recipient_id = Wo_Secure($_GET['recipient_id']);
            deleteDisappearingMessages($recipient_id,$wo['user']['user_id']);
        ...
        if ($wo['config']['who_upload'] == 'pro' && $wo['user']['is_pro'] == 0 && !Wo_IsAdmin() && (!empty($_FILES['sendMessageFile']) || !empty($_POST['message-record']))) {
            ...
            if (true) {
                if (!empty($_POST['user_id']) && Wo_CheckMainSession($hash_id) === true) {
                    ...
                    if (!empty($_POST['chatSticker']) && strpos($_POST['chatSticker'], '/upload/') !== false && isSticker($_POST['chatSticker'])) {
                    ...
                    $group_tab = Wo_GroupTabData($_GET['group_id']);
                    if ($group_tab['type'] == 'channel' && $wo['user']['id'] != $group_tab['user_id']) {
                        header("Content-type: application/json");
                        echo json_encode([
                            'status' => 400,
                            'message' => $wo['lang']['not_channel_owner'],
                        ]);
                        exit();
                    }
                    $message_text = '';
        ...
        if (count($parts) > 0) {
            foreach ($parts as $wo['part']) {
                ...
                $normalized_parts[] = array(
                    ...
                    'avatar_full' => $wo['part']['avatar_full'] ?? ($wo['part']['avatar'] ?? ''),
                    'verified' => (int) ($wo['part']['verified'] ?? 0),
                    'lastseen' => (int) ($wo['part']['lastseen'] ?? 0),
                    'lastseen_status' => $wo['part']['lastseen_status'] ?? '',
                    'lastseen_text' => $wo['part']['lastseen_text'] ?? '',
                    'showlastseen' => (int) ($wo['part']['showlastseen'] ?? 0)
                );
            }
            $data['status'] = 200;
            $data['html']   = $html;
