...
if ($f == 'create_new_video_call') {
    ...
    $message_page_html = '';
    $chat_message_html = '';
    $preview_text = !empty($wo['lang']['video_call']) ? $wo['lang']['video_call'] : 'Video call';
    
    if (empty($_GET['user_id2']) || empty($_GET['user_id1']) || Wo_CheckMainSession($hash_id) === false || $_GET['user_id1'] != $wo['user']['user_id']) {
        exit();
    }
    if (!is_numeric($_GET['user_id2']) || $_GET['user_id2'] < 1 || !is_numeric($_GET['user_id1']) || $_GET['user_id1'] < 1) {
        exit();
    }
    ...
    if ($provider == 'agora') {
        ...
        if ($insertData > 0) {
            ...
            $data = array(
                ...
                'provider' => $provider,
                'id' => $insertData,
                'url' => Wo_BuildCallJoinUrl($room_script, 'video', array(
                    'id' => $insertData,
                    'provider' => $provider
                )),
                'html' => Wo_LoadPage('modals/calling'),
