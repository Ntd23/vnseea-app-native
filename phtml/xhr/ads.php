    ...
    if ($s == 'new' && $wo['loggedin'] === true) {
        ...
        if (empty($error)) {
            ...
            $registration_data             = array(
                ...
                'location' => isset($_POST['location']) ? Wo_Secure($_POST['location']) : '',
                'audience' => !empty($_POST['audience-list'])
              ? Wo_Secure(implode(',', $_POST['audience-list']))
              : '',
            ...
            if($_POST['appears'] == 'story' || $_POST['appears'] == 'entire') {
                $story_data            = array();
                $story_data['ad_id'] = $last_id;
                $story_data['user_id'] =$wo['user']['user_id'];
                $story_data['posted']  = time();
                $story_data['expire']  = time() + (60 * 60 * 24);
    ...
    if ($s == 'update' && $wo['loggedin'] === true) {
        ...
        if (empty($error)) {
            $update_data = array(
                ...
                'description' => Wo_Secure($_POST['description']),
                'location' => isset($_POST['location']) ? Wo_Secure($_POST['location']) : '',
                'audience' => (isset($_POST['audience-list']) && is_array($_POST['audience-list']))
               ? Wo_Secure(implode(',', $_POST['audience-list']))
               : '',
                'gender' => Wo_Secure($_POST['gender']),
                'appears' => Wo_Secure($_POST['appears']),
