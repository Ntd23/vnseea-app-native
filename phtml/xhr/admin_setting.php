...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'verfiy_apps') {
        ...
        if (!empty($_POST['android_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.hisotechgroup.com/access_token.php?code={$android_code}&type=android", false, stream_context_create($arrContextOptions));
        ...
        if (!empty($_POST['windows_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.hisotechgroup.com/access_token.php?code={$windows_code}&type=windows_desktop", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
        ...
        if (!empty($_POST['ios_purchase_code'])) {
            ...
            $file         = file_get_contents("http://www.hisotechgroup.com/access_token.php?code={$windows_code}&type=ios", false, stream_context_create($arrContextOptions));
            $check        = json_decode($file, true);
            if (!empty($check['status'])) {
                if ($check['status'] == 'SUCCESS') {
    ...
    if ($s == 'delete_fake_users' && Wo_CheckMainSession($hash_id) === true) {
        ...
        // echo "</pre>";
        // error_log("delete_fake_users: start");
