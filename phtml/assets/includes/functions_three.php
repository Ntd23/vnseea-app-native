...
function Wo_GetCurrencyRule($currency = 0)
{
	global $wo;
	...
	if (is_numeric($currency_id) && isset($wo['currencies'][(int) $currency_id])) {
		...
		foreach ($wo['currencies'] as $key => $currency_data) {
			if ($currency_data['text'] === $currency_code) {
				$currency_id = $key;
				break;
			}
		}
	...
	if ($currency_code === '') {
		$currency_id     = 0;
		$currency_code   = !empty($wo['currencies'][0]['text']) ? $wo['currencies'][0]['text'] : $wo['config']['currency'];
		$currency_symbol = !empty($wo['currencies'][0]['symbol']) ? $wo['currencies'][0]['symbol'] : Wo_GetCurrency($currency_code);
	}

	$rules = array(
		'USD' => array('decimals' => 2, 'decimal_sep' => '.', 'thousand_sep' => ','),
		'EUR' => array('decimals' => 2, 'decimal_sep' => ',', 'thousand_sep' => '.'),
		'JPY' => array('decimals' => 0, 'decimal_sep' => '', 'thousand_sep' => ','),
		'TRY' => array('decimals' => 2, 'decimal_sep' => ',', 'thousand_sep' => '.'),
		'GBP' => array('decimals' => 2, 'decimal_sep' => '.', 'thousand_sep' => ','),
		'RUB' => array('decimals' => 2, 'decimal_sep' => ',', 'thousand_sep' => ' '),
		'PLN' => array('decimals' => 2, 'decimal_sep' => ',', 'thousand_sep' => ' '),
		'ILS' => array('decimals' => 2, 'decimal_sep' => '.', 'thousand_sep' => ','),
		'BRL' => array('decimals' => 2, 'decimal_sep' => ',', 'thousand_sep' => '.'),
		'INR' => array('decimals' => 2, 'decimal_sep' => '.', 'thousand_sep' => ','),
		'VND' => array('decimals' => 0, 'decimal_sep' => '', 'thousand_sep' => '.')
	);

	$rule = !empty($rules[$currency_code]) ? $rules[$currency_code] : array(
		'decimals' => 2,
		'decimal_sep' => '.',
		'thousand_sep' => ','
	);

	$rule['code'] = $currency_code;
	$rule['id'] = $currency_id;
	$rule['symbol'] = $currency_symbol;
...
function Wo_FormatIndianCurrency($value, $decimals = 2, $decimal_sep = '.', $thousand_sep = ',')
{
	...
	if (strlen($integer_part) > 3) {
		...
		$leading = substr($integer_part, 0, -3);
		$chunks = array();

		while (strlen($leading) > 2) {
			array_unshift($chunks, substr($leading, -2));
			$leading = substr($leading, 0, -2);
		}

		if ($leading !== '') {
			array_unshift($chunks, $leading);
		}

		$integer_part = implode($thousand_sep, $chunks) . $thousand_sep . $last_three;
	}

	if ($decimals > 0) {
		return $integer_part . $decimal_sep . $fraction_part;
	}

	return $integer_part;
}
function Wo_FormatPriceByCurrency($value, $currency = 0)
{
	$rule = Wo_GetCurrencyRule($currency);
	$numeric_value = (float) preg_replace('/[^0-9.\-]/', '', (string) $value);

	if ($rule['code'] === 'INR') {
		return Wo_FormatIndianCurrency($numeric_value, $rule['decimals'], $rule['decimal_sep'], $rule['thousand_sep']);
	}

	return number_format($numeric_value, $rule['decimals'], $rule['decimal_sep'], $rule['thousand_sep']);
}
function Wo_ParsePriceByCurrency($value, $currency = 0)
{
	$rule = Wo_GetCurrencyRule($currency);
	...
	if ($last_dot !== false || $last_comma !== false) {
		if ($last_dot !== false && $last_comma !== false) {
			$candidate_pos = max($last_dot, $last_comma);
			...
			$digits_after = preg_replace('/\D/', '', substr($value, $candidate_pos + 1));
			$digits_before = preg_replace('/\D/', '', substr($value, 0, $candidate_pos));

			if (substr_count($value, $separator) === 1 && $digits_before !== '' && $digits_after !== '' && strlen($digits_after) <= (int) $rule['decimals']) {
				$decimal_pos = $candidate_pos;
			}
		}
	}
	...
	if ($decimal_pos !== false) {
		$integer_part = preg_replace('/\D/', '', substr($value, 0, $decimal_pos));
		$fraction_part = preg_replace('/\D/', '', substr($value, $decimal_pos + 1));

		if ($integer_part === '' && $fraction_part === '') {
			return false;
		}

		if (strlen($fraction_part) > (int) $rule['decimals']) {
			return false;
		}

		$integer_part = ($integer_part === '') ? '0' : ltrim($integer_part, '0');
		$integer_part = ($integer_part === '') ? '0' : $integer_part;
		$fraction_part = str_pad($fraction_part, (int) $rule['decimals'], '0');
...
function Wo_GetSideBarAds()
{
    ...
    WHERE `user_id` IN (SELECT `user_id` FROM " . T_USERS . " WHERE `wallet` > 0)
        ...
        (`gender` = '$user_gender' OR `gender` = 'all')  AND ( `audience` = '' OR `audience` IS NULL OR `audience` LIKE '%$user_country%')
...
function Wo_GetStoryThumb($story_id = 0, $thumbnail = '')
{
	global $sqlConnect, $wo;
	...
	if (!empty($thumbnail)) {
		return array(
			'type' => 'image',
			'filename' => Wo_GetMedia($thumbnail)
		);
	}
	$image_query = mysqli_query($sqlConnect, "SELECT `filename` FROM " . T_USER_STORY_MEDIA . " WHERE `story_id` = {$story_id} AND `type` = 'image' ORDER BY `id` ASC LIMIT 1");
	if ($image_query && mysqli_num_rows($image_query) > 0) {
		$image = mysqli_fetch_assoc($image_query);
		if (!empty($image['filename'])) {
			return array(
				'type' => 'image',
				'filename' => Wo_GetMedia($image['filename'])
			);
		}
	}
	$video_query = mysqli_query($sqlConnect, "SELECT `filename` FROM " . T_USER_STORY_MEDIA . " WHERE `story_id` = {$story_id} AND `type` = 'video' ORDER BY `id` ASC LIMIT 1");
	if ($video_query && mysqli_num_rows($video_query) > 0) {
		...
		if (!empty($video['filename'])) {
			if ($wo['config']['ffmpeg_system'] == 'on' && !empty($wo['config']['ffmpeg_binary_file']) && file_exists($video['filename'])) {
				...
				$image_thumb = $dir . '/' . Wo_GenerateKey() . '_' . $story_id . "_story_thumb.jpeg";
				$output_thumb = shell_exec("$ffmpeg_b -ss \"1\" -i " . $video['filename'] . " -vframes 1 -f mjpeg $image_thumb 2<&1");
				if (file_exists($image_thumb) && !empty(@getimagesize($image_thumb))) {
					Wo_Resize_Crop_Image(400, 400, $image_thumb, $image_thumb, $wo['config']['images_quality']);
					...
					mysqli_query($sqlConnect, "UPDATE " . T_USER_STORY . " SET `thumbnail` = '" . Wo_Secure($image_thumb, 0) . "' WHERE `id` = {$story_id}");
					return array(
						'type' => 'image',
						'filename' => Wo_GetMedia($image_thumb)
					);
				}
			}
			return array(
				'type' => 'video',
				'filename' => Wo_GetMedia($video['filename'])
			);
		}
	}
	return false;
	...
	while ($fetched_data = mysqli_fetch_assoc($query_run)) {
		...
		if (empty($fetched_data['thumb'])) {
			$fetched_data['thumb'] = array(
				'type' => 'image',
				'filename' => Wo_GetMedia($fetched_data['user_data']['avatar_org'])
			);
		}
		$fetched_data['videos']            = Wo_GetStoryMedia($fetched_data['id'], 'video');
		$fetched_data['is_owner']          = ($fetched_data['user_id'] == $wo['user']['id'] || Wo_IsAdmin() || Wo_IsModerator()) ? true : false;
