...
function Wo_GetMessagesUsers($user_id, $searchQuery = '', $limit = 50, $new = false, $update = 0, $source = 'recent')
{
    ...
    $user_id = Wo_Secure($user_id);
    $limit = (is_numeric($limit) && $limit > 0) ? (int) $limit : 50;
    if ($source === 'follow') {
        return Wo_GetFollowedMessageUsers($user_id, $searchQuery, $limit);
    }
    $data = array();
    $excludes = array();
    ...
    if (empty($searchQuery)) {
        $fallback_limit = $limit * 20;
        $query_fallback = "SELECT * FROM " . T_MESSAGES . " WHERE ((`to_id` = {$user_id} AND `deleted_two` = '0') OR (`from_id` = {$user_id} AND `deleted_one` = '0')) AND `page_id` = 0 ORDER BY `time` DESC LIMIT {$fallback_limit}";
        $sql_query_fallback = mysqli_query($sqlConnect, $query_fallback);
        if ($sql_query_fallback && mysqli_num_rows($sql_query_fallback) > 0) {
            $added = array();
            ...
            while ($sql_fetch_fallback = mysqli_fetch_assoc($sql_query_fallback)) {
                $conversation_user_id = ($sql_fetch_fallback['from_id'] == $user_id) ? $sql_fetch_fallback['to_id'] : $sql_fetch_fallback['from_id'];
                $conversation_user_id = (int) $conversation_user_id;
                if ($conversation_user_id < 1 || $conversation_user_id == $user_id) {
                    continue;
                }
                if (isset($added[$conversation_user_id])) {
                    continue;
                }
                if (Wo_IsBlocked($conversation_user_id) === true) {
                    continue;
                }
                $user = Wo_UserData($conversation_user_id);
                if (!empty($user)) {
                    if (!empty($sql_fetch_fallback['time'])) {
                        $user['chat_time'] = $sql_fetch_fallback['time'];
                    }
                    $sql_fetch_fallback['conversation_user_id'] = $conversation_user_id;
                    $user['message'] = $sql_fetch_fallback;
                    $data[] = $user;
                    $added[$conversation_user_id] = true;
                }
            }
        }
        if (!empty($data)) {
...
function Wo_GetGroupCallUrl($group_id = 0, $room_name = '', $call_type = 'video')
{
    global $wo;
    $group_id = (int) $group_id;
    $call_type = ($call_type === 'audio') ? 'audio' : 'video';
    $room_name = preg_replace('/[^A-Za-z0-9_-]/', '', (string) $room_name);
    if ($group_id < 1 || empty($room_name)) {
        return '#';
    }
    return $wo['config']['site_url'] . '/call.php?room=' . urlencode($room_name) . '&type=' . $call_type . '&group_id=' . $group_id;
}

function Wo_GetGroupCallData($text = '')
{
    if (empty($text) || !is_string($text)) {
        return false;
    }
    if (!preg_match('/^\[\[GROUP_CALL\|(audio|video)\|([0-9]+)\|([A-Za-z0-9_-]+)\]\]$/', trim($text), $matches)) {
        return false;
    }
    ...
    return array(
        ...
        'group_id' => $group_id,
        'room_name' => $room_name,
        'title' => ($call_type === 'audio') ? 'Group audio call' : 'Group video call',
        'button_text' => ($call_type === 'audio') ? 'Join audio call' : 'Join video call',
        'url' => Wo_GetGroupCallUrl($group_id, $room_name, $call_type)
    );
}

function Wo_GetGroupCallPreviewText($text = '')
{
    $call_data = Wo_GetGroupCallData($text);
    if ($call_data === false) {
        return $text;
    }
    return $call_data['title'];
}
...
function Wo_CanSendMessageToUser($recipient_id = 0, $user_id = 0)
{
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    if (empty($recipient_id) || !is_numeric($recipient_id) || $recipient_id < 1) {
        return false;
    }
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        $user_id = $wo['user']['user_id'];
    ...
    if (empty($recipient) || empty($recipient['user_id'])) {
        return false;
    }
    return true;
}

function Wo_HasMessageThread($user_id = 0, $recipient_id = 0)
{
    global $sqlConnect;
    if (empty($user_id) || !is_numeric($user_id) || $user_id < 1) {
        return false;
    ...
    $user_id = Wo_Secure($user_id);
    $recipient_id = Wo_Secure($recipient_id);
    $query = mysqli_query($sqlConnect, "SELECT COUNT(`id`) FROM " . T_MESSAGES . " WHERE ((`from_id` = {$user_id} AND `to_id` = {$recipient_id}) OR (`from_id` = {$recipient_id} AND `to_id` = {$user_id}))");
    return (Wo_Sql_Result($query, 0) > 0);
}

function Wo_CanAccessMessageThread($recipient_id = 0, $user_id = 0)
{
    global $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (Wo_CanSendMessageToUser($recipient_id, $user_id) === true) {
        return true;
    }
    if (Wo_CanSendMessageToUser($user_id, $recipient_id) === true) {
        return true;
    }
    return Wo_HasMessageThread($user_id, $recipient_id);
}
...
function Wo_GetMessageButton($user_id = 0)
{
    ...
    if (Wo_CanSendMessageToUser($user_id, $logged_user_id) === true) {
...
function Wo_ListUserTagLabels($owner_id)
{
    global $wo, $sqlConnect, $db;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $query = mysqli_query($sqlConnect, "SELECT * FROM " . T_USER_TAG_LABELS . " WHERE owner_id={$owner_id} ORDER BY id DESC");
    $labels = [];
    while ($row = mysqli_fetch_assoc($query)) {
        $labels[] = $row;
    }
...
function Wo_CreateTagLabel($data = [])
{
    global $wo, $sqlConnect;
    ...
    // (tuỳ chọn) validate màu #RRGGBB
    if (!preg_match('/^#[0-9A-Fa-f]{6}$/', $color)) {
        $color = '#999999';
    }

    // Escape chuỗi
    $nameEsc  = mysqli_real_escape_string($sqlConnect, $name);
    $colorEsc = mysqli_real_escape_string($sqlConnect, $color);

    // CHÚ Ý: name & color PHẢI có dấu nháy đơn
    $sql = "INSERT INTO " . T_USER_TAG_LABELS . " (`owner_id`,`name`,`color`)
            VALUES ($owner_id, '$nameEsc', '$colorEsc')";
...
function Wo_UpdateTagLabel($label_id = null, $data = [])
{
    global $wo, $sqlConnect;
    ...
    if ($label_id < 1) {
        return ['status' => 400, 'message' => 'label_id can not be empty'];
    }
    if ($name === '') {
        return ['status' => 400, 'message' => 'Name can not be empty'];
    }
    ...
    $existing = mysqli_query(
        $sqlConnect,
        "SELECT id FROM " . T_USER_TAG_LABELS . " WHERE owner_id={$owner_id} AND id={$label_id} LIMIT 1"
    );
    if (!$existing) {
        return [
            'status' => 500,
    ...
    $nameEsc = mysqli_real_escape_string($sqlConnect, $name);
    $colorEsc = mysqli_real_escape_string($sqlConnect, $color);
    $sql = "UPDATE " . T_USER_TAG_LABELS . " SET `name`='$nameEsc', `color`='$colorEsc' WHERE owner_id={$owner_id} AND id={$label_id} LIMIT 1";
    $ok = mysqli_query($sqlConnect, $sql);
    if (!$ok) {
        return [
            'status' => 500,
...
function Wo_GetAllAssignedTagsByOwner($owner_id = null)
{
    global $wo, $sqlConnect;
...
function Wo_GetAllTagsForUser($owner_id = null)
{
    global $wo, $sqlConnect;
    if (empty($wo['loggedin'])) return [];
    $owner_id = (int)$wo['user']['user_id'];
    $sql = "SELECT DISTINCT UTA.owner_id,UTA.tag_id,UTL.name,UTL.color FROM " . T_USER_TAG_ASSIGNMENTS . " AS UTA INNER JOIN " . T_USER_TAG_LABELS . " AS UTL ON UTA.tag_id = UTL.id WHERE UTA.owner_id = {$owner_id}";
    $ok = mysqli_query($sqlConnect, $sql);
    if (!$ok) {
        return [
            'status'    => 500,
...
// hàm chọn tag_id lấy ra danh sách id người dùng
function Wo_GetUserIdsByTag($tag_id = null)
{
    global $wo, $sqlConnect;
    if (empty($wo['loggedin'])) return [];
...
// function Wo_CreateTagLabel($owner_id,$name,$color)


// function createLabels($owner_id,$name)
