    ...
    if (empty($error_code)) {
        ...
        if ($register === true) {
            ...
            if ($activate == 1) {
                ...
                $create_access_token = mysqli_query($sqlConnect, "INSERT INTO " . T_APP_SESSIONS . " (`user_id`, `session_id`, `platform`, `platform_details`, `time`) VALUES ('{$user_id}', '{$access_token}', '{$device_type}', '{$platform_details}', '{$time}')");
