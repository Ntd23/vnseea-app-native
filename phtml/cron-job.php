...
if (!empty($expired_stories)) {
    ...
    if (!empty($deleteIDS)) {
        ...
        @mysqli_query($sqlConnect, "DELETE FROM " . T_USER_STORY_MEDIA . " WHERE `story_id` IN (" . $deleteIDS . ")");
        @mysqli_query($sqlConnect, "DELETE FROM " . T_USER_STORY . " WHERE `id` IN (" . $deleteIDS . ")");
    }
...
if ($wo['config']['monetization'] == 1) {
    $subscriptions = $db->where('status', 1)->where('expire', time(), '<')->get(T_MONETIZATION_SUBSCRIBTION);
    if (!empty($subscriptions)) {
        foreach ($subscriptions as $subscription) {
            $wo['user'] = Wo_UserData($subscription->user_id);
            //if (Wo_ShouldSubscriptionBePaid($subscription->monetization_id, $subscription->last_payment_date)) {
                Wo_SubscriptionPay($subscription->monetization_id);
            //}
        }
    }
}
