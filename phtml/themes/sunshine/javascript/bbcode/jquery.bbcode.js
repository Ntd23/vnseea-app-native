...
(function () {
  ...
  _render = function (e, d) {
    ...
    a.color = {
      re: /\[color=([#a-z0-9]+)\]([\s\S]*?)\[\/color\]/gi,
      sub: function (a, b, c) {
        return '<span style="color:' + b + ';">' + c + "</span>";
      },
    };
    a.email = {
      re: /\[email\]([\s\S]*?)\[\/email\]/gi,
      sub: function (a, b) {
    ...
    a.youtube = {
      re: /\[youtube\](?:http?:\/\/)?(?:www\.)?youtu(?:\.be\/|be\.com\/watch\?v=)([A-Z0-9\-_]+)(?:&(.*?))?\[\/youtube\]/gi,
      sub: function (a, b) {
        return (
          '<iframe class="youtube-player" type="text/html" width="640" height="385" src="http://www.youtube.com/embed/' +
          b +
    ...
    return this.each(function () {
      ...
      if ($this.is("textarea")) {
        ...
        $cloned_textarea.bind("focus", function () {
          $cloned_textarea
            .next(".bbcode-preview")
            .html(bbcode.render($cloned_textarea.val()));
        });
        ...
        $editor.find(".bbcode-image").bind("click", function () {
          var img_url = prompt(
            "Enter a valid url to an image",
            "http://i.imgur.com/sJIRJ.gif"
          );
          if (!img_url) {
            return;
          }

          $cloned_textarea.replaceSelectedText(
            "[img]" + img_url + "[/img]",
            "",
            true
          );
        ...
        $editor.find(".bbcode-iframe").bind("click", function () {
          ...
          var pattern4 =
            /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/embed\/([\w\-]{10,12})/g;
          var pattern5 =
            /(?:http?s?:\/\/)?(?:www\.)?(?:vimeo\.com)\/video\/(\d+)/g;
          ...
          if (!url) {
            ...
            var replacement =
              "[IFRAME]https://player.vimeo.com/video/$1[/IFRAME]";
            url = url.replace(pattern2, replacement);
          } else if (pattern3.test(url)) {
            var replacement =
              "[IFRAME]https://www.youtube.com/embed/$1[/IFRAME]";
        ...
        $editor.find(".bbcode-justify").bind("click", function () {
          $cloned_textarea.surroundSelectedText(
            "[justify]",
            "[/justify]",
            true
          );
