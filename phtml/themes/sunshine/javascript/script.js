...
if ('IntersectionObserver' in window) {
    window.wo_video_observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            const postId = entry.target.getAttribute('data-observer-post-id');
            const player = window.wo_players[postId];
            if (player) {
                if (entry.isIntersecting) {
                    console.log("Playing video for post:", postId);
                    player.play().catch(error => {
                        console.log("Autoplay prevented for post " + postId + ":", error);
                    });
                } else {
                    console.log("Pausing video for post:", postId);
                    player.pause();
                }
            }
        });
...
function Wo_ObserveVideo(postId, target) {
    ...
    if (element) {
        ...
        console.log("Started observing video for post:", postId);
    } else {
        console.log("Could not find element to observe for post:", postId, target);
    }
}
...
$(function () {
  ...
  $(window).scroll(function () {
    ...
    if($('#load-more-posts').length > 0 && scrolled == 0) {
        var scrollHeight = Math.max(document.body.scrollHeight, document.documentElement.scrollHeight, document.body.offsetHeight, document.documentElement.offsetHeight, document.body.clientHeight, document.documentElement.clientHeight);
        if ($(window).scrollTop() + $(window).height() > $('#load-more-posts').offset().top - 200 || $(window).scrollTop() + $(window).height() >= scrollHeight - 200) {
             scrolled = 1;
             Wo_GetMorePosts();
        }
    }
  ...
  setInterval(function() {
      if ($('#load-more-posts').length > 0 && scrolled == 0 && !$('body').attr('no-more-posts')) {
          var btn = document.getElementById('load-more-posts');
          if (btn && btn.style.display !== 'none' && btn.style.visibility !== 'hidden') {
              var rect = btn.getBoundingClientRect();
              if (rect.bottom >= -300 && rect.top <= (window.innerHeight || document.documentElement.clientHeight) + 500) {
                  scrolled = 1;
                  Wo_GetMorePosts();
              }
          }
