...
window.wo_players = window.wo_players || {};
window.wo_video_observer = window.wo_video_observer || null;
...
if ('IntersectionObserver' in window && !window.wo_video_observer) {
  window.wo_video_observer = new IntersectionObserver(function(entries) {
    entries.forEach(function(entry) {
      var postId = entry.target.getAttribute('data-observer-post-id');
      var player = window.wo_players[postId];

      if (!player) {
        return;
      }
      if (window.wo_active_fullscreen_post_id) {
          if (window.wo_active_fullscreen_post_id == postId) {
            return;
          }
...
function Wo_TryPlayMediaElement(elementId) {
...
function Wo_ObserveVideo(postId, target) {
  if (!window.wo_video_observer) {
    return;
  }
  ...
  element.setAttribute('data-observer-post-id', postId);
  window.wo_video_observer.observe(element);
}

function Wo_UpdateAutoLoadTriggerState() {
  var more_posts = $('#load-more-posts[data-auto-load-only="1"]');
  if (!more_posts.length) {
    return;
  }

  if ($('body').attr('no-more-posts')) {
    more_posts.removeAttr('style');
    more_posts.find('.btn').removeAttr('style');
    return;
  }
...
window.wo_auto_load_posts_observer = window.wo_auto_load_posts_observer || null;
function Wo_SetupAutoLoadPostsObserver() {
  if (!('IntersectionObserver' in window)) {
    return;
  }

  var more_posts = document.querySelector('#load-more-posts[data-auto-load-only="1"]');
  if (!more_posts) {
    return;
  }
  ...
  window.wo_auto_load_posts_observer = new IntersectionObserver(function(entries) {
    entries.forEach(function(entry) {
      if (entry.isIntersecting && $('#load-more-posts').length > 0 && scrolled == 0 && !$('body').attr('no-more-posts')) {
        scrolled = 1;
        Wo_GetMorePosts();
      }
    });
  ...
  window.wo_auto_load_posts_observer.observe(more_posts);
}
...
function Wo_intervalUpdates(force_update = 0, loop = 0) {
    ...
    ajax_request['_t'] = (new Date()).getTime();
      ...
      if(typeof (data.notifications) != "undefined" && data.notifications > 0) {
        ...
        if(data.notifications != current_notification_number) {
          if (data.notifications_sound == 0) {
            Wo_TryPlayMediaElement('notification-sound');
          }
      ...
      if(typeof (data.messages) != "undefined" && data.messages > 0) {
        ...
        if(data.messages != $("[data_messsages_count]").attr('data_messsages_count')) {
          if (data.notifications_sound == 0 && $("[data_messsages_count]").attr('data_messsages_count') < data.messages) {
            Wo_TryPlayMediaElement('message-sound');
          }
...
function Wo_OpenFirstFilteredVideo(attempt) {
  var tries = attempt || 0;
  var firstVideo = document.querySelector("#posts video[data-post-video]");
  ...
  var player = (window.wo_players && window.wo_players[postId]) || null;

  if (!player) {
    var container = firstVideo.closest(".plyr");
    if (container && window.wo_players) {
      for (var key in window.wo_players) {
        if (
          window.wo_players[key] &&
          window.wo_players[key].elements &&
          window.wo_players[key].elements.container === container
        ) {
          player = window.wo_players[key];
          break;
        }
      }
    }
...
function Wo_OpenChatTab(recipient_id, group_id,product_id = 0,page_id = 0,page_user_id = 0,story_id = 0) {
  CheckIfCanUse();
  var viewport_width = Math.max(window.innerWidth || 0, document.documentElement.clientWidth || 0, $(window).width() || 0);
  if (group_id == null) {
    group_id = 0;
  }

  if ($(".chat_"+recipient_id).length > 0 && story_id == 0) {
    var existing_chat = $(".chat_" + recipient_id);
    var existing_messages_wrapper = existing_chat.find(".chat-messages-wrapper-" + recipient_id);
    SendSeen(recipient_id);
    existing_chat.show();
    existing_chat.closest(".chat_main").show();
    existing_chat.find(".online-toggle-hdr").attr("style", "");
    existing_chat.find(".chat-tab-container-" + recipient_id).stop(true, true).show();
    if (existing_messages_wrapper.length > 0) {
      existing_messages_wrapper.scrollTop(existing_messages_wrapper[0].scrollHeight);
    }
    return false;
  }
  go_to = '';
  ...
  var direct_chat_url = "";
  if (page_id > 0) {
    direct_chat_url = websiteUrl + "/messages/" + page_user_id + "&page=" + page_id;
  } else if (recipient_id > 0) {
    direct_chat_url = websiteUrl + "/messages/" + recipient_id;
    if (story_id != 0) {
      direct_chat_url += "?story_id=" + story_id;
    }
  }
  if ((viewport_width < 720 || $(".chat-tab").length === 0 || $("body").attr("chat-off")) && direct_chat_url !== "" && group_id === 0) {
    document.location = direct_chat_url;
    return false;
  }
  if (story_id != 0) {
    Wo_CloseLightbox();
  ...
  if(node_socket_flow === "0"){
    $.get(Wo_Ajax_Requests_File(), {
      ...
      if(viewport_width < 720 || length > 0) {
        if (page_id > 0) {
  ...
  $.get(Wo_Ajax_Requests_File(), {
    ...
    if(data.status == 200) {
      ...
      loading_div.empty();
      $(document.body).removeAttr('data-chat-recipient-' + recipient_id);
    }
  });
}
...
function Wo_AddEmoToCommentInput(post_id, code, type) {
    inputTag = Wo_GetCommentScope(post_id).find('.comment-textarea');
...
function Wo_PlayAudioCall(type) {
  ...
  if (type == 'play') {
    ...
    if (window.woAudioCallPlayAttemptActive === true || window.woPendingAudioCallSoundRetry === true) {
      return;
    }
    window.woAudioCallPlayAttemptActive = true;
    var audioPlayPromise = audioElement.play();
    if (audioPlayPromise && typeof audioPlayPromise.then === 'function') {
      audioPlayPromise.then(function() {
        window.woAudioCallPlayAttemptActive = false;
      }).catch(function() {
        window.woAudioCallPlayAttemptActive = false;
        if (window.woAudioCallAutoplayLogged !== true) {
          console.log("Autoplay blocked: call sound will play after user interaction.");
          window.woAudioCallAutoplayLogged = true;
        }
        if (window.woPendingAudioCallSoundRetry === true) {
          return;
        }
        window.woPendingAudioCallSoundRetry = true;
        window.woAudioCallResumeHandler = function() {
          var resumeHandler = window.woAudioCallResumeHandler;
          window.woPendingAudioCallSoundRetry = false;
          window.woAudioCallAutoplayLogged = false;
          window.woAudioCallResumeHandler = null;
        ...
        document.addEventListener('click', window.woAudioCallResumeHandler, true);
        document.addEventListener('touchstart', window.woAudioCallResumeHandler, true);
        document.addEventListener('keydown', window.woAudioCallResumeHandler, true);
      });
    } else {
      window.woAudioCallPlayAttemptActive = false;
    }
    ...
    window.woPendingAudioCallSoundRetry = false;
    window.woAudioCallAutoplayLogged = false;
    if (window.woAudioCallResumeHandler) {
      document.removeEventListener('click', window.woAudioCallResumeHandler, true);
      document.removeEventListener('touchstart', window.woAudioCallResumeHandler, true);
      document.removeEventListener('keydown', window.woAudioCallResumeHandler, true);
      window.woAudioCallResumeHandler = null;
    }
    if (audioElement) {
      audioElement.pause();
...
function Wo_PlayVideoCall(type) {
  var videoAudioElement = document.getElementById('video-calling-sound');
  if (type == 'play') {
    if (!videoAudioElement) {
      return;
    }
    if (window.woVideoCallPlayAttemptActive === true || window.woPendingVideoCallSoundRetry === true) {
      return;
    }
    window.woVideoCallPlayAttemptActive = true;
    var videoPlayPromise = videoAudioElement.play();
    if (videoPlayPromise && typeof videoPlayPromise.then === 'function') {
      videoPlayPromise.then(function() {
        window.woVideoCallPlayAttemptActive = false;
      }).catch(function() {
        window.woVideoCallPlayAttemptActive = false;
        if (window.woVideoCallAutoplayLogged !== true) {
          console.log("Autoplay blocked: video call sound will play after user interaction.");
          window.woVideoCallAutoplayLogged = true;
        }
        if (window.woPendingVideoCallSoundRetry === true) {
          return;
        }
        window.woPendingVideoCallSoundRetry = true;
        window.woVideoCallResumeHandler = function() {
          var resumeHandler = window.woVideoCallResumeHandler;
          window.woPendingVideoCallSoundRetry = false;
          window.woVideoCallAutoplayLogged = false;
          window.woVideoCallResumeHandler = null;
          document.removeEventListener('click', resumeHandler, true);
          document.removeEventListener('touchstart', resumeHandler, true);
          document.removeEventListener('keydown', resumeHandler, true);
          Wo_PlayVideoCall('play');
        };
        document.addEventListener('click', window.woVideoCallResumeHandler, true);
        document.addEventListener('touchstart', window.woVideoCallResumeHandler, true);
        document.addEventListener('keydown', window.woVideoCallResumeHandler, true);
      });
    } else {
      window.woVideoCallPlayAttemptActive = false;
    }
    ...
    window.woPendingVideoCallSoundRetry = false;
    window.woVideoCallAutoplayLogged = false;
    if (window.woVideoCallResumeHandler) {
      document.removeEventListener('click', window.woVideoCallResumeHandler, true);
      document.removeEventListener('touchstart', window.woVideoCallResumeHandler, true);
      document.removeEventListener('keydown', window.woVideoCallResumeHandler, true);
      window.woVideoCallResumeHandler = null;
    }
    if (videoAudioElement) {
...
function Wo_UploadCommentImage(id) {
  var image_container = Wo_GetCommentScope(id);
...
/* ===== Fullscreen Video Action Buttons ===== */
(function() {
  var REACTIONS = [
    {emoji:'👍', name:'1', label:'Like'},
    {emoji:'❤️', name:'2', label:'Love'},
    {emoji:'😂', name:'3', label:'Haha'},
    {emoji:'😮', name:'4', label:'Wow'},
    {emoji:'😢', name:'5', label:'Sad'},
    {emoji:'😡', name:'6', label:'Angry'}
  ];

  var ICON_LIKE = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M23,10C23,8.89 22.1,8 21,8H14.68L15.64,3.43C15.66,3.33 15.67,3.22 15.67,3.11C15.67,2.7 15.5,2.32 15.23,2.05L14.17,1L7.59,7.58C7.22,7.95 7,8.45 7,9V19A2,2 0 0,0 9,21H18C18.83,21 19.54,20.5 19.84,19.78L22.86,12.73C22.95,12.5 23,12.26 23,12V10M1,21H5V9H1V21Z"></path></svg>';
  var ICON_COMMENT = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M9,22A1,1 0 0,1 8,21V18H4A2,2 0 0,1 2,16V4C2,2.89 2.9,2 4,2H20A2,2 0 0,1 22,4V16A2,2 0 0,1 20,18H13.9L10.2,21.71C10,21.9 9.75,22 9.5,22V22H9M5,5V7H19V5H5M5,9V11H13V9H5M5,13V15H15V13H5Z"></path></svg>';
  var ICON_SHARE = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="currentColor" d="M21,12L14,5V9C7,10 4,15 3,20C5.5,16.5 9,14.9 14,14.9V19L21,12Z"></path></svg>';
  var ICON_SAVE = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path fill="currentColor" d="M200-120v-640q0-33 23.5-56.5T280-840h400q33 0 56.5 23.5T760-760v640L480-240 200-120Zm80-122 200-86 200 86v-518H280v518Zm0-518h400-400Z"/></svg>';
  var ICON_SAVE_FILLED = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960"><path fill="currentColor" d="M200-120v-640q0-33 23.5-56.5T280-840h400q33 0 56.5 23.5T760-760v640L480-240 200-120Z"/></svg>';

  function getPostIdFromPlyr(plyrEl) {
    var video = plyrEl.querySelector('video');
    if (!video) return null;
    var postId = video.getAttribute('data-post-video');
    if (postId) return postId;
    var id = video.getAttribute('id');
    if (id && id.indexOf('video-') === 0) {
      return id.replace(/^video-+/, ''); 
    }
    if (window.wo_players) {
      for (var key in window.wo_players) {
        if (window.wo_players[key] && window.wo_players[key].elements && 
            window.wo_players[key].elements.container === plyrEl) {
          return key;
        }
      }
    }
  ...
  var overlayCommentSheetState = window.wo_video_overlay_comment_sheet_state || {
    shell: null,
    panel: null,
    content: null,
    activePostId: null,
  ...
  window.wo_video_overlay_comment_sheet_state = overlayCommentSheetState;
  ...
  function getPlayerContainer(player) {
    ...
    return player.elements.container || null;
  }

  function removeFullscreenDecorations(scope) {
    var root = scope || document;
    var nodes = root.querySelectorAll(
      ".video-fullscreen-actions, .video-fullscreen-info, .vfa-toast",
    );
  ...
  function ensureOverlayCommentSheet() {
    if (
      overlayCommentSheetState.shell &&
      overlayCommentSheetState.panel &&
      overlayCommentSheetState.content
    ) {
      return overlayCommentSheetState.shell;
    }

    var shell = document.createElement("div");
    shell.className = "wo-video-comment-sheet";
    shell.innerHTML =
      '<div class="wo-video-comment-sheet__backdrop"></div>' +
      '<div class="wo-video-comment-sheet__panel" role="dialog" aria-modal="true">' +
      '<div class="wo-video-comment-sheet__grab"></div>' +
      '<div class="wo-video-comment-sheet__content"></div>' +
      "</div>";

    document.body.appendChild(shell);

    overlayCommentSheetState.shell = shell;
    overlayCommentSheetState.panel = shell.querySelector(
      ".wo-video-comment-sheet__panel",
    );
    overlayCommentSheetState.content = shell.querySelector(
      ".wo-video-comment-sheet__content",
    );

    ["touchstart", "touchmove", "touchend", "wheel"].forEach(
      function (eventName) {
        overlayCommentSheetState.panel.addEventListener(eventName, function (e) {
          e.stopPropagation();
        });
      },
    );
    ...
    shell
      .querySelector(".wo-video-comment-sheet__backdrop")
  ...
  function closeOverlayCommentSheet() {
    var state = overlayCommentSheetState;

    if (state.activeNode && state.placeholder && state.placeholder.parentNode) {
      state.placeholder.parentNode.insertBefore(state.activeNode, state.placeholder);
      state.placeholder.parentNode.removeChild(state.placeholder);
      if (state.wasHidden) {
        state.activeNode.classList.add("hidden");
      }
    }
  ...
  function openOverlayCommentSheet(postId) {
    var post = document.getElementById("post-" + postId);
    if (!post) {
      return;
    }

    var commentNode = post.querySelector(".post-comments");
    if (!commentNode) {
      return;
    }

    ensureOverlayCommentSheet();
    closeOverlayCommentSheet();

    var placeholder = document.createElement("div");
    placeholder.className = "wo-video-comment-sheet-placeholder";
    commentNode.parentNode.insertBefore(placeholder, commentNode);

    overlayCommentSheetState.activePostId = postId;
    overlayCommentSheetState.activeNode = commentNode;
    overlayCommentSheetState.placeholder = placeholder;
    overlayCommentSheetState.wasHidden = commentNode.classList.contains("hidden");

    commentNode.classList.remove("hidden");
    overlayCommentSheetState.content.innerHTML = "";
    overlayCommentSheetState.content.appendChild(commentNode);
    overlayCommentSheetState.shell.classList.add("is-active");

    setTimeout(function () {
      if (typeof Wo_ShowCommentCombo === "function") {
        Wo_ShowCommentCombo(postId);
      }
      var textarea = commentNode.querySelector("textarea");
      if (textarea) {
        textarea.focus();
      }
    }, 120);
  ...
  function ensureOverlayShell() {
    if (overlayState.shell && overlayState.stage) {
      return overlayState.shell;
    }

    var shell = document.createElement("div");
    shell.className = "wo-video-fullscreen-shell";
    shell.innerHTML =
      '<div class="wo-video-fullscreen-stage" aria-live="polite"></div>';
    document.body.appendChild(shell);

    overlayState.shell = shell;
    ...
    shell.addEventListener(
      ...
      function (e) {
        ...
        if ($(e.target).closest(".plyr__controls, .video-fullscreen-actions").length) {
          return;
        }
    ...
    shell.addEventListener(
      "touchstart",
      function (e) {
        if (!overlayState.activePlayer || !e.touches || !e.touches.length) {
          return;
        }
        if ($(e.target).closest(".plyr__controls, .video-fullscreen-actions").length) {
          overlayState.touchTracking = false;
          return;
        }

        overlayState.touchTracking = true;
        overlayState.touchStartY = e.touches[0].clientY;
    ...
    shell.addEventListener(
      ...
      function (e) {
        if (
          !overlayState.activePlayer ||
          !overlayState.touchTracking ||
          !e.touches ||
          !e.touches.length
        ) {
          return;
        ...
        var moveY = e.touches[0].clientY - overlayState.touchStartY;
        var moveX = e.touches[0].clientX - overlayState.touchStartX;
    ...
    shell.addEventListener(
      ...
      function (e) {
        ...
        var deltaY = e.changedTouches[0].clientY - overlayState.touchStartY;
        var deltaX = e.changedTouches[0].clientX - overlayState.touchStartX;

        overlayState.touchTracking = false;
        overlayState.touchStartY = null;
        overlayState.touchStartX = null;
    ...
    if (!overlayState.escBound) {
      document.addEventListener("keydown", function (e) {
        if (e.key === "Escape" && overlayState.activePlayer) {
          e.preventDefault();
      ...
      window.addEventListener("popstate", function () {
        ...
        overlayState.historyPushed = false;
        closeActiveVideoOverlay(false);

        setTimeout(function () {
          if (typeof Wo_FilterPostBy === "function") {
            Wo_FilterPostBy("all");
          }
        }, 60);
      });

      overlayState.escBound = true;
    }

    return shell;
  }

  function getFeedVideoOrder() {
    var order = [];
    var seen = {};
    var items = [];
    var videos = document.querySelectorAll("video[data-post-video]");

    for (var i = 0; i < videos.length; i++) {
      var postId = videos[i].getAttribute("data-post-video");
      if (!postId || seen[postId]) {
        continue;
      }

      var postNode = document.getElementById("post-" + postId);
      var anchorNode = postNode || videos[i].closest(".post") || videos[i];
    ...
    for (var j = 0; j < items.length; j++) {
      order.push(items[j].postId);
    }

    return order;
  }

  function updateOverlayButtonState(container, isActive) {
    if (!container) return;

    container.classList.toggle("plyr--fullscreen-active", !!isActive);
    container.classList.toggle("wo-video-overlay-player", !!isActive);
  ...
  function createOverlayPlaceholder(container, postId) {
    var placeholder = document.createElement("div");
    var rect = container.getBoundingClientRect();
    placeholder.className = "wo-video-overlay-placeholder";
    placeholder.setAttribute("data-post-id", postId || "");
    placeholder.style.height = Math.max(container.offsetHeight, rect.height) + "px";
  ...
  function restorePlayerToFeed(player) {
    ...
    if (player.__wo_overlay_media && player.__wo_overlay_ended_handler) {
      player.__wo_overlay_media.removeEventListener(
        "ended",
        player.__wo_overlay_ended_handler,
      );
    }
    player.__wo_overlay_media = null;
    player.__wo_overlay_ended_handler = null;

    removeFullscreenDecorations(container);
    updateOverlayButtonState(container, false);

    var placeholder = container.__wo_overlay_placeholder;
    if (placeholder && placeholder.parentNode) {
      placeholder.parentNode.insertBefore(container, placeholder);
      placeholder.parentNode.removeChild(placeholder);
    }

    container.__wo_overlay_placeholder = null;
  }
  ...
  function openVideoOverlay(player) {
    ...
    if (overlayState.activePlayer === player && overlayState.shell) {
      overlayState.shell.classList.add("is-active");
      return true;
    }

    if (overlayState.activePlayer && overlayState.activePlayer !== player) {
      restorePlayerToFeed(overlayState.activePlayer);
    }

    overlayState.feedOrder = getFeedVideoOrder();

    if (!container.__wo_overlay_placeholder && container.parentNode) {
      var placeholder = createOverlayPlaceholder(container, postId);
      container.parentNode.insertBefore(placeholder, container);
      container.__wo_overlay_placeholder = placeholder;
    }

    overlayState.stage.innerHTML = "";
    overlayState.stage.appendChild(container);
    overlayState.shell.classList.add("is-active");

    overlayState.activePlayer = player;
    overlayState.activePostId = postId;
    window.wo_active_fullscreen_post_id = postId;

    if (
      !overlayState.historyPushed &&
      window.history &&
      typeof window.history.pushState === "function"
    ) {
      window.history.pushState(
        { woVideoOverlay: true, postId: postId },
        "",
        window.location.href,
      );
      overlayState.historyPushed = true;
    }

    var mediaEl = container.querySelector("video");
    if (mediaEl) {
      if (player.__wo_overlay_media && player.__wo_overlay_ended_handler) {
        player.__wo_overlay_media.removeEventListener(
          "ended",
  ...
  function closeActiveVideoOverlay(shouldResumeObservedVideos) {
    ...
    overlayState.activePlayer = null;
    overlayState.activePostId = null;
    window.wo_active_fullscreen_post_id = null;
    overlayState.historyPushed = false;
    ...
    if (overlayState.shell) {
      overlayState.shell.classList.remove("is-active");
    }

    if (shouldResumeObservedVideos !== false) {
      resumeObservedVideos();
    }
  }

  function scrollToVideoPost(postId) {
    var post = document.getElementById("post-" + postId);
    ...
    if (
      ...
      typeof player.elements.container.scrollIntoView === "function"
    ) {
      player.elements.container.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    }
  ...
  function getPlayerByPostId(postId) {
    if (!postId) return null;
    if (window.wo_players[postId]) {
      return window.wo_players[postId];
    }

    var videoEl = document.querySelector(
      'video[data-post-video="' + postId + '"]',
    );
    return resolvePlayerFromVideo(videoEl);
  }

  function waitForPlayerByPostId(postId, callback, attempt) {
    var tries = attempt || 0;
    ...
    setTimeout(function () {
      waitForPlayerByPostId(postId, callback, tries + 1);
    }, 180);
  }

  function resolvePlayerFromVideo(videoEl) {
    if (!videoEl || !window.wo_players) return null;
    var container = videoEl.closest(".plyr");
  ...
  function playNextFullscreenVideo(currentPlayer) {
    ...
    var currentPostIdFromEvent =
      (currentPlayer && currentPlayer.__wo_post_id) ||
      getPostIdFromPlyr(currentContainer);

    if (
      !overlayState.activePlayer ||
      (overlayState.activePlayer !== currentPlayer &&
    ...
    if (overlayState.activePlayer && overlayState.activePlayer !== currentPlayer) {
      currentPlayer = overlayState.activePlayer;
      currentContainer = getPlayerContainer(currentPlayer);
    }
    ...
    var nextPostId = null;
    for (var i = currentIndex + 1; i < feedOrder.length; i++) {
      if (String(feedOrder[i] || "") !== String(overlayState.activePostId || "")) {
        nextPostId = feedOrder[i];
        break;
      }
    }
    ...
    switchOverlayToPost(nextPostId, currentPlayer);
  }

  window.Wo_PlayNextFullscreenVideo = playNextFullscreenVideo;
  window.Wo_CloseActiveVideoOverlay = closeActiveVideoOverlay;

  function enhancePlayerFullscreen(player) {
    if (!player || player.__wo_overlay_bound) {
      return;
    }
    ...
    if (player.fullscreen) {
      try {
        Object.defineProperty(player.fullscreen, "active", {
          configurable: true,
          get: function () {
            return overlayState.activePlayer === player;
          },
        });
      } catch (err) {}

      player.fullscreen.enter = function () {
    ...
    if (fullscreenBtn && !fullscreenBtn.__wo_overlay_click_bound) {
      fullscreenBtn.__wo_overlay_click_bound = true;
  ...
  window.Wo_EnableVideoOverlayFullscreen = enhancePlayerFullscreen;

  function resumeObservedVideos() {
    if (!window.wo_video_observer) return;
    var observed = document.querySelectorAll("[data-observer-post-id]");
    for (var i = 0; i < observed.length; i++) {
      var postId = observed[i].getAttribute("data-observer-post-id");
      var player = window.wo_players[postId];
      if (!player) continue;
      ...
      var visibleArea = Math.max(0, visibleHeight) * Math.max(0, visibleWidth);
  ...
  function showToast(plyrEl, msg) {
    var old = plyrEl.querySelector('.vfa-toast');
    if (old) old.remove();
    var t = document.createElement('div');
    t.className = 'vfa-toast';
    t.textContent = msg;
    plyrEl.appendChild(t);
    setTimeout(function(){ t.classList.add('show'); }, 10);
    setTimeout(function(){ t.classList.remove('show'); setTimeout(function(){ t.remove(); }, 300); }, 1500);
  }

   function getPostContainer(postId) {
    return document.getElementById("post-" + postId);
  }

  function getInlineReactionElement(postId, reaction) {
    var post = getPostContainer(postId);
    if (!post) return null;
    return post.querySelector(
      '.reaction[data-post-id="' +
        postId +
        '"][data-reaction-id="' +
        reaction +
        '"]',
  ...
  function getInlineReactionIcon(element) {
    if (!element) return "";
    var image = element.querySelector("img");
    return image ? image.getAttribute("src") : "";
  }

  function syncFullscreenReactionButton(plyrEl, reaction, emoji, label) {
    var button = plyrEl.querySelector(
      '.video-fullscreen-actions [data-action="like"]',
    );
    if (!button) return;
    button.classList.add("active-reaction");
    button.setAttribute("data-current-reaction", reaction);
    button.innerHTML = '<span class="vfa-emoji-active">' + emoji + "</span>";
    if (label) {
      showToast(plyrEl, label);
    }
  }

  function registerFullscreenReaction(postId, reaction, plyrEl, emoji, label) {
    var inlineReaction = getInlineReactionElement(postId, reaction);

    if (inlineReaction && typeof Wo_RegisterReaction === "function") {
      Wo_RegisterReaction(
        inlineReaction,
        getInlineReactionIcon(inlineReaction),
        0,
      );
      syncFullscreenReactionButton(plyrEl, reaction, emoji, label);
      return;
    }
    ...
    $.get(
      ...
      function (data) {
        if (data.status == 200) {
          var post = getPostContainer(postId);
          if (post && data.reactions) {
            $(post)
              .find(".post-reactions-icons-" + postId)
              .html(data.reactions);
          }
          syncFullscreenReactionButton(plyrEl, reaction, emoji, label);
        }
 ...
 function createFullscreenButtons(plyrEl) {
    ...
    var isFollowing = video.getAttribute("data-is-following") === "1";
    var isOwnVideo = video.getAttribute("data-is-own-video") === "1";
    var vidTitle = video.getAttribute("data-video-title") || "";
    var postCaption = video.getAttribute("data-post-caption") || "";

    // Action buttons (right side)
    var div = document.createElement("div");
    div.className = "video-fullscreen-actions";

    // Build reactions popup
    var reactHTML = '<div class="vfa-reactions-popup">';
    for (var i = 0; i < REACTIONS.length; i++) {
      reactHTML +=
        '<span class="vfa-reaction-item" data-reaction="' +
        REACTIONS[i].name +
        '" title="' +
        REACTIONS[i].label +
        '">' +
    ...
    div.innerHTML =
      '<div class="vfa-btn-wrap">' +
      reactHTML +
      '<div class="vfa-btn pointer" data-action="like">' +
      ICON_LIKE +
      "</div>" +
      "</div>" +
      '<div class="vfa-btn pointer" data-action="comment">' +
      ICON_COMMENT +
      "</div>" +
      '<div class="vfa-btn pointer" data-action="share">' +
      ICON_SHARE +
      "</div>" +
      '<div class="vfa-btn pointer" data-action="save">' +
      ICON_SAVE +
      "</div>";

    plyrEl.appendChild(div);

    // Info overlay (bottom left)
    var video = plyrEl.querySelector("video");
    var pubName = video.getAttribute("data-user-name") || "";
    var pubUrl = video.getAttribute("data-user-url") || "#";
    var pubAvatar = video.getAttribute("data-user-avatar") || "";
    var pubId = video.getAttribute("data-user-id") || "";
    var isFollowing = video.getAttribute("data-is-following") === "1";
    var isOwnVideo = video.getAttribute("data-is-own-video") === "1";
    var vidTitle = video.getAttribute("data-video-title") || "";
    var postCaption = video.getAttribute("data-post-caption") || "";
    var displayCaption = postCaption || vidTitle;

    var info = document.createElement("div");
    info.className = "video-fullscreen-info";
    var followBtn =
      isFollowing || isOwnVideo
        ? ""
        : '<button class="vfa-follow-btn">Theo dõi</button>';

    info.innerHTML =
      '<div class="vfa-user-row">' +
      '<a href="' +
      pubUrl +
      '" class="vfa-user-link">' +
      '<img src="' +
      pubAvatar +
      '" class="vfa-avatar" style="width:44px !important; height:44px !important; min-width:44px !important; min-height:44px !important; border-radius:50% !important; object-fit:cover !important; border:2px solid #fff !important; display:block !important; flex: 0 0 44px !important; flex-shrink: 0 !important;">' +
      "</a>" +
      '<div class="vfa-user-copy">' +
      '<a href="' +
      pubUrl +
      '" class="vfa-user-link">' +
      '<span class="vfa-username">' +
      pubName +
      "</span>" +
      "</a>" +
      '<div class="vfa-video-title">' +
      displayCaption +
      "</div>" +
      "</div>" +
      followBtn +
    ...
    plyrEl.appendChild(info);

    // Khi click vào link user, thoát fullscreen trước để chuyển trang mượt hơn
    info.querySelectorAll(".vfa-user-link").forEach(function (link) {
    ...
    var fBtn = info.querySelector(".vfa-follow-btn");
    if (fBtn) {
      fBtn.addEventListener("click", function (e) {
        e.stopPropagation();
        if (typeof Wo_RegisterFollow === "function") {
          Wo_RegisterFollow(pubId);
          fBtn.remove();
          showToast(plyrEl, "Following " + pubName);
        }
      });
    }

    // --- Like: show/hide reactions on hover ---
    var likeWrap = div.querySelector(".vfa-btn-wrap");
    var reactPopup = div.querySelector(".vfa-reactions-popup");
    var hideTimeout;
    var touchTimer;

    // Prevent native context menu on all fullscreen buttons
    ...
    likeWrap.addEventListener("mouseenter", function () {
      clearTimeout(hideTimeout);
      reactPopup.classList.add("show");
    });
    likeWrap.addEventListener("mouseleave", function () {
      hideTimeout = setTimeout(function () {
        reactPopup.classList.remove("show");
      }, 400);
    });

    // Mobile: long-press (500ms) to show reactions
    likeWrap.addEventListener(
      "touchstart",
      function (e) {
        e.preventDefault();
    ...
    likeWrap.addEventListener("touchend", function (e) {
      clearTimeout(touchTimer);
      // If popup not showing, it was a quick tap = simple like
      if (!reactPopup.classList.contains("show")) {
        div.querySelector('[data-action="like"]').click();
      }
    });
    // Hide reactions when touching outside
    plyrEl.addEventListener("touchstart", function (e) {
      if (!likeWrap.contains(e.target) && !reactPopup.contains(e.target)) {
        reactPopup.classList.remove("show");
      }
    });

    // Click reaction emoji
    var reactionItems = div.querySelectorAll(".vfa-reaction-item");
    for (var r = 0; r < reactionItems.length; r++) {
      (function (item) {
        function doReaction(e) {
          e.stopPropagation();
          var reaction = item.getAttribute("data-reaction");
          reactPopup.classList.remove("show");
          registerFullscreenReaction(
            postId,
            reaction,
            plyrEl,
            item.textContent,
        ...
        item.addEventListener("click", doReaction);
        // Mobile: touchstart stops propagation (prevents popup close), touchend fires reaction
    ...
    // Simple like click (tap without hover)
    div
      .querySelector('[data-action="like"]')
      .addEventListener("click", function (e) {
        e.stopPropagation();
        registerFullscreenReaction(postId, "1", plyrEl, "👍", "Liked!");
      });
    ...
    div
      ...
      .addEventListener("click", function (e) {
        e.stopPropagation();
        openOverlayCommentSheet(postId);
      });
    ...
    div
      ...
      .addEventListener("click", function (e) {
        ...
        closeActiveVideoOverlay(false);
        setTimeout(function () {
          if (typeof Wo_SharePostOn === "function")
            Wo_SharePostOn(postId, 0, "share");
          else if (typeof Wo_SharePost === "function") Wo_SharePost(postId, 0);
        }, 400);
      });

    // Save with visual feedback
    var saveBtn = div.querySelector('[data-action="save"]');
    saveBtn.addEventListener("click", function (e) {
      e.stopPropagation();
      var self = this;
      $.get(
        Wo_Ajax_Requests_File(),
        { f: "posts", s: "save_post", post_id: postId },
        function (data) {
          if (data.status == 200) {
            self.innerHTML = ICON_SAVE_FILLED;
            self.classList.add("active-saved");
            showToast(plyrEl, "Saved!");
          } else if (data.status == 300) {
            self.innerHTML = ICON_SAVE;
            self.classList.remove("active-saved");
            showToast(plyrEl, "Unsaved");
          }
        },
      );
    });
   ...
   document.addEventListener("fullscreenchange", function () {
    ...
    if (fsEl && fsEl.classList && fsEl.classList.contains("plyr")) {
      var fsPostId = getPostIdFromPlyr(fsEl);
      window.wo_active_fullscreen_post_id = fsPostId;
      if (fsPostId) {
        pauseOtherPlayers(fsPostId);
      }
      createFullscreenButtons(fsEl);
    } else {
      window.wo_active_fullscreen_post_id = null;
      removeFullscreenDecorations(document);
      resumeObservedVideos();
    }
  });
  ...
  document.addEventListener("webkitfullscreenchange", function () {
    ...
    var fsEl = document.webkitFullscreenElement;
    if (fsEl && fsEl.classList && fsEl.classList.contains("plyr")) {
      var fsPostId = getPostIdFromPlyr(fsEl);
      window.wo_active_fullscreen_post_id = fsPostId;
      if (fsPostId) {
