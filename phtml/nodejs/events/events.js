...
module.exports.privateMessageToPersonOwnerFalse = async (ctx, io, data, fromUser, nextId, hasHTML, sendable_message, color) => {
    ...
    io.to(data.to_id).emit('private_message', {
        ...
        message_page_html: await compiledTemplates.messageListOwnerFalse(ctx, data, sendable_message, fromUser, hasHTML, sendable_message),
        id: ctx.userHashUserId[data.from_id],
        receiver: data.to_id,
        username: ((fromUser && fromUser.first_name !== undefined && fromUser.first_name != '') ? fromUser.first_name : fromUser.username),
...
module.exports.privateMessagePageToPersonOwnerFalseWithMedia = async (ctx, io, data, fromUser, nextId, hasHTML, isSticker) => {
    ...
    await io.to(data.to_id).emit('private_message_page', {
        ...
        message_page_html: await compiledTemplates.messageListOwnerFalseWithMedia(ctx, data, nextId, fromUser, isSticker),
        status: 200,
        id: ctx.userHashUserId[data.from_id],
        receiver: data.to_id,
        sender: ctx.userHashUserId[data.from_id],
...
module.exports.groupMessage = async (ctx, io, socket, data, messageOwner, nextId, hasHTML, sendable_message,responseData) => {
    for (let client of Object.keys(io.sockets.adapter.rooms["group" + data.group_id].sockets)) {
        ...
        if (ctx.userIdSocket[messageOwner.user_id].filter(d => d.id === client).length) {
            responseData.html = await compiledTemplates.groupListOwnerTrue(ctx, messageOwner, nextId, data, hasHTML, sendable_message);
            responseData.message_page_html = await compiledTemplates.messageListOwnerTrue(ctx, data, messageOwner, nextId, hasHTML, sendable_message, data.color);
            await io.to(client).emit('group_message', responseData);
        } else {
            responseData.html = await compiledTemplates.groupListOwnerFalse(ctx, messageOwner, nextId, data, hasHTML, sendable_message);
            responseData.message_page_html = await compiledTemplates.messageListOwnerTrue(ctx, data, messageOwner, nextId, hasHTML, sendable_message, data.color);
            await io.to(client).emit('group_message', responseData);
...
module.exports.groupMessageWithMedia = async (ctx, io, socket, data, messageOwner, nextId, isSticker,responseData) => {
    for (let client of Object.keys(io.sockets.adapter.rooms["group" + data.group_id].sockets)) {
        if (client === socket.id) {
            continue;
        }
        if (ctx.userIdSocket[messageOwner.user_id].filter(d => d.id === client).length) {
            responseData.html = await compiledTemplates.groupListOwnerTrueWithMedia(ctx, messageOwner, nextId, data, isSticker);
            await io.to(client).emit('group_message', responseData);
        } else {
            responseData.html = await compiledTemplates.groupListOwnerFalseWithMedia(ctx, messageOwner, nextId, data, isSticker);
            await io.to(client).emit('group_message', responseData);
        }
        let open_groups = Object.entries(ctx.userIdGroupChatOpen).filter(d => d[1] == data.group_id);
        for (let item of open_groups) {
...
module.exports.groupMessagePage = async (ctx, io, socket, data, messageOwner, nextId, hasHTML, sendable_message,responseData) => {
    for (let client of Object.keys(io.sockets.adapter.rooms["group" + data.group_id].sockets)) {
        if (client === socket.id) {
            continue;
        }
        if (ctx.userIdSocket[messageOwner.user_id].filter(d => d.id === client).length) {
            responseData.message_html = await compiledTemplates.messageListOwnerTrue(ctx, data, messageOwner, nextId, hasHTML, sendable_message);
            responseData.message_page_html = await compiledTemplates.messageListOwnerTrue(ctx, data, messageOwner, nextId, hasHTML, sendable_message, data.color);
            responseData.self = true;
            await io.to(client).emit('group_message_page', responseData);
            ...
            responseData.message_html = await compiledTemplates.messageListOwnerFalse(ctx, data, nextId, messageOwner, hasHTML, sendable_message);
            responseData.message_page_html = await compiledTemplates.messageListOwnerFalse(ctx, data, nextId, messageOwner, hasHTML, sendable_message);
            responseData.self = false;
            await io.to(client).emit('group_message_page', responseData);
            if (ctx.socketIdUserHash[client] && ctx.userHashUserId[ctx.socketIdUserHash[client]]) {
                await updateMessageGroupsListInternal(ctx, io, ctx.userHashUserId[data.from_id], ctx.userHashUserId[ctx.socketIdUserHash[client]], sendable_message, data.group_id)
            }
...
module.exports.groupMessagePageWithMedia = async (ctx, io, socket, data, messageOwner, nextId, hasHTML, sendable_message, isSticker,responseData) => {
    for (let client of Object.keys(io.sockets.adapter.rooms["group" + data.group_id].sockets)) {
        if (client === socket.id) {
            continue;
        }
        if (ctx.userIdSocket[messageOwner.user_id].filter(d => d.id === client).length) {
            responseData.message_page_html = await compiledTemplates.messageListOwnerTrueWithMedia(ctx, data, nextId, messageOwner, isSticker);
            responseData.self = true;
            await io.to(client).emit('group_message_page', responseData);
        } else {
            responseData.message_page_html = await compiledTemplates.messageListOwnerFalseWithMedia(ctx, data, nextId, messageOwner, hasHTML, isSticker);
            responseData.self = false;
            await io.to(client).emit('group_message_page', responseData);
        }
        let open_groups = Object.entries(ctx.userIdGroupChatOpen).filter(d => d[1] == data.group_id);
