...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $original_story_text = '';
    if (isset($wo['story']['Orginaltext']) && is_string($wo['story']['Orginaltext'])) {
        $original_story_text = $wo['story']['Orginaltext'];
    }
    elseif (isset($wo['story']['postText']) && is_string($wo['story']['postText'])) {
        $original_story_text = $wo['story']['postText'];
    }
    $wo['description']   = Wo_Secure(mb_substr($original_story_text, 0, 156, "utf-8"));
...
if (!empty($wo['description'])) {
    $wo['title'] =  FilterStripTags(Wo_Secure(Wo_GetShortTitle($original_story_text, false, 50)));
} else {
