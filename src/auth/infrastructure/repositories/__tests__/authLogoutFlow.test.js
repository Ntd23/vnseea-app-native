...
describe('logout flow cleanup and navigation', () => {
  it('cleans local auth state even when backend logout fails', () => {
    const source = read('src/auth/infrastructure/repositories/ApiAuthRepository.ts');
    ...
    expect(source).toContain("const AUTH_DEBUG_PREFIX = '[VNSEEA_AUTH_DEBUG]';");
    ...
    expect(logoutBlock).toContain("logAuthDebug('auth_logout_local_cleanup_done'");
  ...
  it('resets root navigation to login from the auth view model after logout', () => {
    ...
    expect(source).toContain("const AUTH_DEBUG_PREFIX = '[VNSEEA_AUTH_DEBUG]';");
    expect(source).toContain("import { navigationRef } from '../../../navigation/navigationRef';");
    ...
    expect(logoutBlock).toContain('await runAuthAction(() => repository.logout())');
    ...
    expect(source).toContain("logAuthDebug('auth_logout_navigation_reset_skipped'");
  });
  ...
  it('keeps logout entrypoints on the centralized auth reset flow', () => {
    ...
    const settingsSource = read('src/settings/presentation/screens/SettingsScreen.tsx');
    const drawerLogoutIndex = drawerSource.indexOf('const handleLogout = useCallback');
  ...
  it('opts out OneSignal on logout and opts in again when identifying a logged-in user', () => {
    ...
    expect(identifyBlock).toContain('OneSignal.User.pushSubscription.optIn();');
    expect(identifyBlock).toContain("logPushDebug('push_opt_in_complete'");
  ...
  it('logs backend logout token/device cleanup without exposing raw access tokens', () => {
    ...
    expect(source).toContain("Wo_VnseeaLogoutDebugLog('logout_device_ids_cleared'");
