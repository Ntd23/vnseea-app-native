...
function ReelItemBase({
  ...
  const isOwnVideo =
    currentUserId && String(item.publisher.userId) === String(currentUserId);
  ...
  const resolvedCommentsPreviewHeight = useMemo(
    () => Math.max(1, Math.min(height, commentsPreviewHeight ?? height * 0.36)),
  ...
  const videoFrameAnimatedStyle = useAnimatedStyle(() => ({
    width:
      fullVideoRect.width +
      (previewVideoRect.width - fullVideoRect.width) *
        commentsPreviewProgress.value,
    height:
      fullVideoRect.height +
      (previewVideoRect.height - fullVideoRect.height) *
        commentsPreviewProgress.value,
    left:
      fullVideoRect.left +
      (previewVideoRect.left - fullVideoRect.left) *
        commentsPreviewProgress.value,
    top:
      fullVideoRect.top +
      (previewVideoRect.top - fullVideoRect.top) *
        commentsPreviewProgress.value,
  }));
  ...
  const endSuppressionTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  }, [initialSeekTime, item.id, item.videoUrl]);
  ...
  useEffect(() => {
    ...
    Image.getSize(
      thumbnailUrl,
      (width, imageHeight) => {
        if (cancelled || width <= 0 || imageHeight <= 0) return;
        setVideoNaturalAspectRatio(current => current ?? width / imageHeight);
      },
      () => {},
    );
  ...
  }, [item.id, item.thumbnailUrl, item.videoUrl]);
  ...
  const triggerHeartBurst = useCallback((x: number, y: number) => {
    ...
    setFloatingHearts(prev => [
      ...prev,
      { id, x, y, rotate: randomRotate, anim },
    ]);
  ...
  return (
    <Animated.View style={[styles.reelRoot, { height }]}>
      <Animated.View style={[styles.mediaStage, mediaStageAnimatedStyle]}>
        {/* Fill the physical screen with a blurred copy while the foreground
          media remains fully visible at its natural aspect ratio. */}
        {item.thumbnailUrl ? (
          <Animated.View
            pointerEvents="none"
          ...
          <View style={styles.avatarWrap}>
            ...
            {showFollowBadge && (
              <TouchableOpacity
                ...
                delayPressIn={0}
                onPress={() =>
                  item.publisher.userId && onFollow?.(item.publisher.userId)
                }
...
function MusicDisc({
  ...
  return (
    <RNAnimated.View
      style={[styles.musicDisc, { transform: [{ rotate: spin }] }]}
    >
