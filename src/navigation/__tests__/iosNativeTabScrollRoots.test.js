...
describe('iOS native tab scroll roots', () => {
  ...
  it('publishes scroll intent from scrolling tabs and hides chrome for full-screen tabs', () => {
    ...
    expect(reelsSource).not.toContain(
  ...
  it('contains every Reels video over a blurred backdrop while keeping one video player', () => {
    const reelItemSource = read(
      'src/reels/presentation/components/ReelItem.tsx',
    );
    const fitSource = read('src/reels/presentation/components/reelVideoFit.ts');

    expect(fitSource).toContain('getContainedReelVideoRect');
    expect(fitSource).toContain('REEL_VIDEO_FALLBACK_ASPECT_RATIO = 9 / 16');
